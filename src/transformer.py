...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # start token (used to inference the first patch)
        self.z_sp = nn.Parameter(torch.normal(mean=0.0, std=0.02, size=(1, config.n_embd)))
    ...
    def forward(self, c_idx, z_idx, mask=None):
        ...
        mask = mask.unsqueeze(1)  # [B,1,2L,2L]
